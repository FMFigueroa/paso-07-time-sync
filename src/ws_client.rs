// ─── Paso 7: Módulo WebSocket Client — heredado de paso-06, reusado para time-sync ───
//
...
impl WsClient {
    pub fn new(light_state: Arc<Mutex<LightState>>) -> Result<Self> {
        ...
        thread::Builder::new()
            ...
            .spawn(move || {
                ws_connection_loop(outbound_rx, light_state, self_tx);
            })?;
...
fn ws_connection_loop(
    ...
    loop {
        ...
        match connect_and_run(&outbound_rx, &light_state, &self_tx) {
            ...
            match opcode {
                WS_OPCODE_TEXT => {
                    if let Ok(text) = std::str::from_utf8(&read_buf[..bytes_read as usize]) {
                        handle_text_frame(text, light_state, self_tx);
                    }
                }
                WS_OPCODE_PING => {
